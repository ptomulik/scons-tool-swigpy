...
class develop(setuptools.command.develop.develop):
    def _make_symlinks(self, sources):
        here = os.path.abspath(os.path.dirname(__file__))
        subdir = os.path.join(here, 'sconstool', 'swigpy')
...
setup(
        ...
        package_dir={'sconstool.swigpy': '.'},
        packages=['sconstool.swigpy'],
        namespace_packages=['sconstool'],
        description='SCons tool to generate Python modules using SWIG',
        long_description=readme,
        long_description_content_type='text/x-rst',
        url='https://github.com/ptomulik/scons-tool-swigpy',
        author='Paweł Tomulik',
        author_email='ptomulik@meil.pw.edu.pl',
        cmdclass={'develop': develop, 'install': install},
        install_requires=['scons-tool-util>=0.1.9'],
        python_requires='>=2.7'
)
